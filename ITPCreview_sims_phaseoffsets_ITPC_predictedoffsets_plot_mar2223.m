...
set(gca,'xlim',[0.5 3.5],...
        ...
        'FontSize',font_tit-2);
ylabel('ITPC','FontSize',font_tit-2);
[pval_itpc_scz,~,stats_itpc_scz] = ranksum(tmp(:,1),tmp(:,2));
[pval_itpc_mdd,~,stats_itpc_mdd] = ranksum(tmp(:,1),tmp(:,3));
...
[pval_phase_scz,~,stats_phase_scz] = ranksum(tmp(:,1),tmp(:,2));
[pval_phase_mdd,~,stats_phase_mdd] = ranksum(tmp(:,1),tmp(:,3));
if ranksum(tmp(:,1),tmp(:,2)) < 0.05
    sigstar({[1,2]},0.05);
end
if ranksum(tmp(:,1),tmp(:,3)) > 0.05
...
[~,~,~,adjp_itpc] = fdr_bh([pval_itpc_scz, pval_itpc_mdd],.05,'pdep','yes');
[~,~,~,adjp_phase] = fdr_bh([pval_phase_scz, pval_phase_mdd],.05,'pdep','yes');

%%
clear; clc;
load(fullfile('E:\EEGdata\Knott_data\Knott_P300_data\','ITPCsim_100offsets_50parts_jan0323.mat'));
